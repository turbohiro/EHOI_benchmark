...
class CustomNet(nn.Module):
    def __init__(self, in_channels =128, channels=128):
        super().__init__()
        self.attention = TemporalWiseAttention(T=16,reduction= 4, dimension = 4)
        self.layer = Conv2d(channels, channels, kernel_size=3, padding=1, bias=False)
...
class TemporalSpatialWiseResidualAttention(nn.Module,base.MultiStepModule):
    def __init__(self,T: int, reduction: int = 16, dimension: int = 4, num_channels: int = 64):
        super().__init__()
        self.step_mode = 'm'
        assert dimension == 4 or dimension == 2, 'dimension must be 4 or 2'
    ...
    def forward(self, x_seq: torch.Tensor):
        assert x_seq.dim() == 3 or x_seq.dim() == 5, ValueError(
            f'expected 3D or 5D input with shape [T, N, M] or [T, N, C, H, W], but got input with shape {x_seq.shape}')
        x_seq = x_seq.transpose(0, 1)
        avgout_temporal = self.sharedMLP(self.avg_pool(x_seq).view([x_seq.shape[0], x_seq.shape[1]]))
        maxout_temporal = self.sharedMLP(self.max_pool(x_seq).view([x_seq.shape[0], x_seq.shape[1]]))

        squeeze_spatial = x_seq.view(x_seq.shape[0],-1,x_seq.shape[2], x_seq.shape[3], x_seq.shape[4]).mean(1)
        avgout_spatial = self.conv(squeeze_spatial)
        scores_temporal = self.sigmoid(avgout_temporal + maxout_temporal)
        scores_spatial =  self.sigmoid(avgout_spatial)

        if self.dimension == 2:
            y_seq = x_seq * scores_temporal[:, :, None]
        elif self.dimension == 4:
            y_seq_temporal = x_seq * scores_temporal[:, :, None, None, None]
            y_seq_spatial = x_seq * scores_spatial[:, None, :, :, :]
        y_seq = torch.max(y_seq_temporal,y_seq_spatial)    
        y_seq = y_seq.transpose(0, 1)
