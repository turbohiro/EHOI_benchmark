import torch
import torch.nn as nn
from copy import deepcopy
from .. import layer
# Incorporating Learnable Membrane Time Constant to Enhance Learning of Spiking Neural Networks https://arxiv.org/abs/2007.05785

__all__ = ['MNISTNet', 'FashionMNISTNet', 'NMNISTNet', 'CIFAR10Net', 'CIFAR10DVSNet', 'DVSGestureNet']


class MNISTNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        self.conv_fc = nn.Sequential(
            layer.Conv2d(1, channels, kernel_size=3, padding=1, bias=False),
            layer.BatchNorm2d(channels),
            spiking_neuron(**deepcopy(kwargs)),
            layer.MaxPool2d(2, 2),

            layer.Conv2d(channels, channels, kernel_size=3, padding=1, bias=False),
            layer.BatchNorm2d(channels),
            spiking_neuron(**deepcopy(kwargs)),
            layer.MaxPool2d(2, 2),

            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 7 * 7, 2048),
            spiking_neuron(**deepcopy(kwargs)),
            layer.Dropout(0.5),
            layer.Linear(2048, 100),
            spiking_neuron(**deepcopy(kwargs)),
            layer.VotingLayer()
        )


    def forward(self, x: torch.Tensor):
        return self.conv_fc(x)


class FashionMNISTNet(MNISTNet):
    pass


class NMNISTNet(MNISTNet):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__(channels, spiking_neuron, **kwargs)
        self.conv_fc[0] = layer.Conv2d(2, channels, kernel_size=3, padding=1, bias=False)
        self.conv_fc[-6] = layer.Linear(channels * 8 * 8, 2048)

class CIFAR10Net(nn.Module):
    def __init__(self, channels=256, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(2):
            for j in range(3):
                if conv.__len__() == 0:
                    in_channels = 3
                else:
                    in_channels = channels

                conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False))
                conv.append(layer.BatchNorm2d(channels))
                conv.append(spiking_neuron(**deepcopy(kwargs)))

            conv.append(layer.MaxPool2d(2, 2))


        self.conv_fc = nn.Sequential(
            *conv,
            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 8 * 8, 2048),
            spiking_neuron(**deepcopy(kwargs)),

            layer.Dropout(0.5),
            layer.Linear(2048, 100),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)
        )

    def forward(self, x):
        return self.conv_fc(x)


class CIFAR10DVSNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(4):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False))
            conv.append(layer.BatchNorm2d(channels))
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            conv.append(layer.MaxPool2d(2, 2))

        self.conv_fc = nn.Sequential(
            *conv,

            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 8 * 8, 512),
            spiking_neuron(**deepcopy(kwargs)),

            layer.Dropout(0.5),
            layer.Linear(512, 100),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)
        )

    def forward(self, x: torch.Tensor):
        return self.conv_fc(x)



class DVSGestureNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(5):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False))
            conv.append(layer.CustomNet(in_channels,channels))
            conv.append(layer.BatchNorm2d(channels))           
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            conv.append(layer.MaxPool2d(2, 2))


        self.conv_fc = nn.Sequential(
            *conv,

            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 4 * 4, 512),
            spiking_neuron(**deepcopy(kwargs)),

            layer.Dropout(0.5),
            layer.Linear(512, 110),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)
        )

    def forward(self, x: torch.Tensor):
        return self.conv_fc(x)

def test_models():
    import torch
    from .. import neuron, surrogate, functional
    x = torch.rand([2, 1, 28, 28])
    net = MNISTNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 1, 28, 28])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

    x = torch.rand([2, 1, 28, 28])
    net = FashionMNISTNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 1, 28, 28])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

    x = torch.rand([2, 2, 32, 32])
    net = NMNISTNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 2, 32, 32])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

    x = torch.rand([2, 3, 32, 32])
    net = CIFAR10Net(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 3, 32, 32])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

    x = torch.rand([2, 2, 128, 128])
    net = CIFAR10DVSNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 2, 128, 128])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

    x = torch.rand([2, 2, 128, 128])
    net = DVSGestureNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 2, 128, 128])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

class DVSHandObjectNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(5):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False))
            #conv.append(layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 4, dimension = 4))
            conv.append(layer.BatchNorm2d(channels))
            
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            conv.append(layer.MaxPool2d(2, 2))


        self.conv_fc = nn.Sequential(
            *conv,
            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 8 * 10, 512),
            spiking_neuron(**deepcopy(kwargs)),

            layer.Dropout(0.5),
            layer.Linear(512, 80),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)       
        )

    def forward(self, x: torch.Tensor):
        x1 = self.conv_fc(x)
        return x1

class DVSHandObjectSynapseNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(5):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=8, padding=1, bias=False))   
            conv.append(layer.BatchNorm2d(channels))
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            conv.append(layer.MaxPool2d(2, 2))
            



        self.conv_fc = nn.Sequential(
            *conv,
            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 3 * 5, 512),
            spiking_neuron(**deepcopy(kwargs)),
            layer.SynapseFilter(tau=10.,learnable=True),

            layer.Dropout(0.5),
            layer.Linear(512, 80),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)
        )

    def forward(self, x: torch.Tensor):
        x1 = self.conv_fc(x)
        return x1


class DVSHandObjectAttentionNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(5):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False)) 
            conv.append(layer.BatchNorm2d(channels))  
            conv.append(layer.TemporalWiseAttention(T=16,reduction= 4, dimension = 4)) 
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            conv.append(layer.MaxPool2d(2, 2))



        self.conv_fc = nn.Sequential(
            *conv,
            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 8 * 10, 512),
            spiking_neuron(**deepcopy(kwargs)),
            

            layer.Dropout(0.5),
            layer.Linear(512, 80),
            spiking_neuron(**deepcopy(kwargs)),

            layer.VotingLayer(10)
        )

    def forward(self, x: torch.Tensor):
        x1 = self.conv_fc(x)
        return x1

class DVSHandObjectSRNNNet(nn.Module):
    def __init__(self, channels=128, spiking_neuron: callable = None, **kwargs):
        super().__init__()

        conv = []
        for i in range(5):
            if conv.__len__() == 0:
                in_channels = 2
            else:
                in_channels = channels

            conv.append(layer.Conv2d(in_channels, channels, kernel_size=3, padding=1, bias=False))   
            conv.append(layer.BatchNorm2d(channels))
            conv.append(spiking_neuron(**deepcopy(kwargs)))
            #conv.append(layer.SynapseFilter(tau=50.))
            conv.append(layer.MaxPool2d(2, 2))

        self.conv_fc = nn.Sequential(
            *conv,
            layer.Flatten(),
            layer.Dropout(0.5),
            layer.Linear(channels * 8 * 10, 512),
            spiking_neuron(**deepcopy(kwargs)),
            layer.LinearRecurrentContainer(
                spiking_neuron(**deepcopy(kwargs)),
                in_features=512, out_features=512, bias=True
            ),
            
            #layer.SynapseFilter(tau=10.,learnable=True),

            layer.Dropout(0.5),
            layer.Linear(512, 80),
            spiking_neuron(**deepcopy(kwargs)),
            layer.LinearRecurrentContainer(
                spiking_neuron(**deepcopy(kwargs)),
                in_features=80, out_features=80, bias=True
            ),
            #layer.SynapseFilter(tau=10.,learnable=True),
            layer.VotingLayer(10)
        )

    def forward(self, x: torch.Tensor):
        x1 = self.conv_fc(x)
        return x1

def main():
    import torch
    from .. import neuron, surrogate, functional
    x = torch.rand([2, 2, 260, 346])
    net = DVSHandObjectNet(16, neuron.IFNode, surrogate_function=surrogate.ATan())
    print(net)
    print(net(x).shape)
    functional.reset_net(net)
    functional.set_step_mode(net, 'm')
    x = torch.rand([4, 2, 2, 260, 346])
    print(net(x).shape)
    functional.reset_net(net)
    del net
    del x

def tput(name):
    import torch
    from .. import neuron, surrogate, functional
    import time
    with torch.no_grad():
        input = torch.rand([16, 1, 2, 260, 346])
        model = DVSHandObjectSRNNNet(16, neuron.ParametricLIFNode, surrogate_function=surrogate.ATan())
        functional.reset_net(model)
        functional.set_step_mode(model, 'm')
        if torch.cuda.is_available():
            input=input.to('cuda')
            model=model.to('cuda')
        model.eval()
        model(input)
        T = 0
        for _ in range(5):
            t1 = time.time()
            model(input)
            t2 = time.time()
            T += (t2-t1)
        T /= 5
        print("Parameter numbers: {}".format(sum(p.numel() for p in model.parameters())))
    print('Forward throughput: %10s : %6.2fms' % (name, T*100))

if __name__ == '__main__':
   tput("DVSHandObject")
    





