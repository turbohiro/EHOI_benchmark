...
class SpikingResNet(nn.Module):
    def __init__(self, block, layers, num_classes=8, zero_init_residual=False,
        ...
        self.conv1 = layer.Conv2d(2, self.inplanes, kernel_size=7, stride=2, padding=3,
                               bias=False)
...
class SpikingResNetAttention(nn.Module):#hand object 8; gesture 11
    def __init__(self, block, layers, num_classes=8, zero_init_residual=False,
                 groups=1, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, spiking_neuron: callable = None, **kwargs):
        super(SpikingResNetAttention, self).__init__()
        if norm_layer is None:
            norm_layer = layer.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        self.maxpool = layer.MaxPool2d(kernel_size=3, stride=2, padding=1)
        self.layer1 = self._make_layer(block, 64, layers[0], spiking_neuron=spiking_neuron, **kwargs)
        self.attentionlayer1 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 64) 
        self.layer2 = self._make_layer(block, 128, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], spiking_neuron=spiking_neuron, **kwargs)
        self.attentionlayer2 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 64)  #64
        self.attentionlayer3 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 128)  #128

        #self.attentionlayer = layer.TemporalWiseAttention(T=16,reduction= 16, dimension = 4) 
        #self.tsattentionlayer1 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 64) 
        #self.tsattentionlayer2 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 128) 
        #self.tsattentionlayer3 = layer.TemporalSpatialWiseResidualAttention(T=16,reduction= 16, dimension = 4, num_channels = 256) 
    ...
    def _forward_impl_me(self, x):
        ...
        x2 = self.attentionlayer1(x0)  #x1 or x0
        x_ = torch.add(x1,x2)  #torch.Size([16, 2, 64, 65, 87])
        #x_ = x1
        x3 = self.layer2(x_)  #torch.Size([16, 2, 128, 33, 44])
        x4 = self.attentionlayer2(x_) #torch.Size([16, 2, 128, 33, 44])
        x4 = self.layer2(x4) 
        x__ = torch.add(x3,x4) #torch.Size([16, 2, 128, 33, 44])
        #x__ = x3

        x5 = self.layer3(x__) #torch.Size([16, 2, 256, 17, 22])
        x6 = self.attentionlayer3(x__)
        x6 = self.layer3(x6) 
        x___ = torch.add(x5,x6)
        #x___ = x5
        x = self.layer4(x___)
    ...
    # def _forward_impl_t_no_res(self, x):
    #     # See note [TorchScript super()]
    #     x = self.conv1(x)
    #     x = self.bn1(x)
    #     x = self.sn1(x)
    ...
    #     x = self.layer1(x)
    #     x = self.attentionlayer(x)  #x1 or x0

    #     x = self.layer2(x)  #torch.Size([16, 2, 128, 33, 44])
    #     x = self.attentionlayer(x) #torch.Size([16, 2, 128, 33, 44])

    #     x = self.layer3(x) #torch.Size([16, 2, 256, 17, 22])
    ...
    # def _forward_impl_ts_no_res(self, x):
    #     # See note [TorchScript super()]
    #     x = self.conv1(x)
    #     x = self.bn1(x)
    #     x = self.sn1(x)
    ...
    #     x = self.layer1(x)
    #     x = self.tsattentionlayer1(x)  #x1 or x0

    #     x = self.layer2(x)  #torch.Size([16, 2, 128, 33, 44])
    #     x = self.tsattentionlayer2(x) #torch.Size([16, 2, 128, 33, 44])

    #     x = self.layer3(x) #torch.Size([16, 2, 256, 17, 22])
    #     x = self.tsattentionlayer3(x)
    #     x = self.layer4(x)
    ...
    def forward(self, x):
        return self._forward_impl_me(x)
...
def _spiking_resnet_attention(arch, block, layers, pretrained, progress, spiking_neuron, **kwargs):
    model = SpikingResNetAttention(block, layers, spiking_neuron=spiking_neuron, **kwargs)
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict)
...
def tput(name):
    ...
    with torch.no_grad():   
        input = torch.rand([16, 1, 2, 260, 346])
        #model = spiking_resnet34(pretrained=False, spiking_neuron=neuron.ParametricLIFNode, surrogate_function=surrogate.ATan(), detach_reset=True)
        model = parametric_lif_net.DVSHandObjectNet(16, neuron.ParametricLIFNode, surrogate_function=surrogate.ATan())
        ...
        # prof.end_profile()
        # gflop_dict, _ = flop_count(model, input)
